...
Uruchom z katalogu streamlit_app:  streamlit run app.py
"""
...
from backend import PORT, SETTINGS, StreamlitGlucoseMonitor
...
# Odświeżenie strony co 10 s (nowy przebieg skryptu — stan w session_state zostaje)
...
st.caption(
    f"Konfiguracja: `../settings.ini` · port **{PORT}** · podsieć `{SETTINGS['subnet_prefix']}` · IP: **{snap.get('ip', '—')}**"
)
...
for i, (label, path) in enumerate(endpoints):
    with cols[i % 3]:
        st.link_button(label, f"{base}{path}", use_container_width=True)
